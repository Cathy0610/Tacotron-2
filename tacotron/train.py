	...
	with tf.variable_scope('Tacotron_model', reuse=tf.AUTO_REUSE) as scope:
		...
		if hparams.predict_linear:
			model.initialize(feeder.eval_inputs, feeder.eval_input_lengths, feeder.eval_mel_targets, feeder.eval_token_targets,
				...
				is_training=False, is_evaluating=True, split_infos=feeder.eval_split_infos, input_emo_labels=eval_feeder.input_emo_labels)
		else:
			model.initialize(feeder.eval_inputs, feeder.eval_input_lengths, feeder.eval_mel_targets, feeder.eval_token_targets,
		...
		try:
			...
			while not coord.should_stop() and step < args.tacotron_train_steps:
				...
				if step % args.eval_interval == 0:
					...
					if hparams.lpc_util:
						audio.inv_lpc_feature(mel_p, os.path.join(eval_wav_dir, 'step-{}-eval-wave-from-mel.wav'.format(step)), hparams)
					else:
						if hparams.GL_on_GPU:
				...
				if step % args.checkpoint_interval == 0 or step == args.tacotron_train_steps or step == 300:
					...
					log('Input at step {}: {}'.format(step, sequence_to_text(input_seq, hparams.tacotron_lang)))
